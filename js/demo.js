...
document.getElementById("template").onchange = function() {
  ...
  TEMPLATE_QUERIES[selected].variables.forEach(function(entry) {
    ...
    var service = encodeURI(
        SPARQL_ENDPOINT + VARIABLE_QUERIES[entry] + '&format=json').replace(
        /#/g, "%23");
...
document.getElementById("process").onclick = function() {
  ...
  if (!isValueMissing) {
    ...
    var service = encodeURI(SPARQL_ENDPOINT + query + '&format=json').replace(
        /#/g, '%23');
    $("#infoPanel")
        .html(
            '<strong>Info :</strong> Some queries take more time to process, thanks for being patient');
      ...
      if (head.indexOf("URI") === -1 && value[head] !== undefined) {
        ...
        if (resource !== undefined) {
          ...
          table += '<td><a target="_blank" href="' + resourceURI
              ...
              + '"> <span property="rdfs:label">' + displayName
              + '</span></a></td>';
        } else {
